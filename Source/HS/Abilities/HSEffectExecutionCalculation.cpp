...
struct HSDamageStatics
{
	...
	HSDamageStatics()
	{
		...
		DEFINE_ATTRIBUTE_CAPTUREDEF(UHSAttributeSetBase, Armor, Target, false);

		// The Target's received Damage. This is the value of health that will be subtracted on the Target. We're not capturing this.
		DEFINE_ATTRIBUTE_CAPTUREDEF(UHSAttributeSetBase, Damage, Target, false);
	}
};

static const HSDamageStatics& DamageStatics()
{
	static HSDamageStatics DStatics;
	return DStatics;
}

UHSEffectExecutionCalculation::UHSEffectExecutionCalculation()
{
	RelevantAttributesToCapture.Add(DamageStatics().ArmorDef);
	// We don't include Damage here because we're not capturing it. It is generated inside the ExecCalc.
}

void UHSEffectExecutionCalculation::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, 
	OUT FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	UHSAbilitySystemComponent* TargetASC = Cast<UHSAbilitySystemComponent>(TargetAbilitySystemComponent);
	if (TargetASC)
	{
		UHSAbilitySystemComponent* SourceASC = Cast<UHSAbilitySystemComponent>(SourceAbilitySystemComponent);
